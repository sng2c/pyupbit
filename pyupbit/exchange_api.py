...
from pyupbit.request_api import (
    _send_get_request,
    _send_post_request,
...
def get_tick_size(price, ticker, method="floor"):
    """원화마켓 주문 가격 단위
...
class Upbit:
    ...
    #    개별 주문 조회
    def get_order(
        self, ticker_or_uuid, state="wait", page=1, limit=100, contain_req=False
    ):
        """
        주문 리스트 조회
        :param ticker: market
    ...
    def get_individual_withdraw_order(
        self, uuid: str, currency: str, contain_req=False
    ):
        """
        개별 출금 조회
        :param uuid: 출금 UUID
